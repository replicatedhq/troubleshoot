...
func (e *MultipleSDKSecretsError) Error() string {
	return fmt.Sprintf("found %d Replicated SDK secrets; use --app / --sdk-namespace on the CLI, or set secretName/secretNamespace in the spec", len(e.Matches))
}
...
func PromptForSDKSecret(matches []SDKSecretMatch) (*ReplicatedUploadCredentials, error) {
	...
	return nil, fmt.Errorf("multiple SDK secrets found; use --app=<slug> or --sdk-namespace=<namespace> on the CLI, or set secretName/secretNamespace in the spec")
}
