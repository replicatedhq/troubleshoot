...
func (c *CollectCertificate) Collect(progressChan chan<- interface{}) (CollectorResult, error) {
	...
	certificate := CertificateCollector(c.Collector.CollectorName, c.Client)

	results := certificate

	filePath := "certificates/" + c.Collector.CollectorName + ".json"
...
func CertificateCollector(secretName string, client kubernetes.Interface) []byte {
